      ...
      <AnimatePresence mode="wait">
        {!sent ? (
          <motion.div key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            ...
            <h3>Запазете консултация</h3>
            <p className="booking-card__lead">Опишете казуса накратко. Ще получите потвърждение за час и следващи стъпки.</p>
            ...
            <form onSubmit={handleSubmit(onSubmit)} noValidate>
              ...
              <button className="booking-submit" type="submit" disabled={isSubmitting}>{isSubmitting ? 'Изпращане...' : 'Изпратете заявка →'}</button>
            </form>
          </motion.div>
        ) : (
          <motion.div key="success" className="booking-success" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
            <div className="booking-success__icon">✓</div>
            <h3>Заявката е получена</h3>
